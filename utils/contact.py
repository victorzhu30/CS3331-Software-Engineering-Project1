...
功能描述:
    识别不同类型的联系方式（邮箱、手机号、QQ号等）
    为每种类型生成相应的可点击链接或显示格式
    支持协议: mailto:, tel:, tencent://
"""
...
def format_contact(contact):
    ...
    功能说明:
        自动识别联系方式的类型（邮箱/手机/QQ等），并生成
        带有相应协议链接的 HTML 代码，支持一键打开对应应用
    
    输入参数:
        contact (str): 用户输入的联系方式字符串
                      例如: "example@qq.com", "13800138000", "123456789"
    ...
    识别规则:
        1. 邮箱: 包含 @ 符号，符合邮箱格式
        2. 手机: 11位数字，以1开头，第二位为3-9
        3. QQ号: 5-11位纯数字
        4. 其他: 作为普通文本显示（如微信号）
    
    协议说明:
        - mailto: 邮件协议，打开默认邮件客户端
        - tel: 电话协议，移动端拨打电话
        - tencent: QQ私有协议，打开QQ临时会话
