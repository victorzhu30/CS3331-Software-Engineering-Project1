...
def _category_field_initial_props(category):
    ...
    for i in range(MAX_DYNAMIC_FIELDS):
        if i < len(defs):
            ...
            required = d.get("required", False)
            props.append(
                {"label": f"{label}{'*' if required else ''}", "visible": True}
            )
        else:
...
def add_item(name, category, description, address, contact, image, *dynamic_values):
    print(
        f"Adding item: {name}, {category}, {description}, {address}, {contact}, {image}"
    )
...
def get_items_list():
    ...
    for item in items:
        ...
        if item.get("image") and os.path.exists(item["image"]):
            ...
            image_tag = (
                f'<img src="gradio_api/file={image_abs_path}" class="item-image" />'
            )
        else:
            # 无图片时显示占位符
        ...
        # 渲染动态属性（不同类别不同字段）
        attrs_html = _render_attributes_html(
            item.get("category", ""), item.get("attributes")
        )
...
def _init_category_tab(request: gr.Request | None):
    ...
    return (
        ...
        gr.update(choices=cats, value=None),
    )
...
def admin_category_save(
    selected_category: str, new_name: str, fields_json: str, request: gr.Request | None
):
    if not _is_admin_request(request):
...
with gr.Blocks(title="物品复活平台 - 首页", css=custom_css) as main_ui:
    ...
    with gr.Tab(label="📝 添加物品"):
        with gr.Row():
            ...
            with gr.Column():
                ...
                _default_cat = (
                    "书籍" if "书籍" in _cats else (_cats[0] if _cats else None)
                )
                ...
                add_desc = gr.Textbox(
                    label="物品描述", placeholder="描述物品的状态、价格等", lines=3
                )
                add_address = gr.Textbox(
                    label="物品地址", placeholder="例如：某某市某某区某某街道"
                )
                add_contact = gr.Textbox(
                    label="联系方式*", placeholder="例如：微信号、QQ号、手机号"
                )
    ...
    with gr.Tab(label="🗑️ 删除物品"):
        ...
        # 绑定删除按钮事件
        del_btn.click(
            delete_item, inputs=[del_id], outputs=[del_output, del_list, del_id]
        )
        ...
        with gr.Row():
            ...
            with gr.Column():
                search_keyword = gr.Textbox(
                    label="搜索关键词", placeholder="输入物品名称或描述"
                )
    ...
    with gr.Tab(label="✅ 用户审批 (管理员)", visible=False) as admin_tab:
        ...
        with gr.Row():
            with gr.Column(scale=1):
                approve_username = gr.Textbox(
                    label="待批准用户名", placeholder="例如：new_user"
                )
    ...
    with gr.Tab(label="🛠️ 物品类型管理 (管理员)", visible=False) as category_admin_tab:
        gr.Markdown(
            "仅管理员可见：新增/修改物品类型（名称 + 属性定义）。默认读取 constants.py；保存后写入 category_config.json。"
        )
        ...
        with gr.Row():
            with gr.Column(scale=1):
                ...
                cat_name = gr.Textbox(
                    label="修改/新增类型名称", placeholder="例如：家具"
                )
            with gr.Column(scale=2):
...
with gr.Blocks(title="物品复活平台 - 用户注册", css=custom_css) as register_page:
    ...
    gr.Markdown(
        value="注册后默认进入待审批 (pending) 状态，管理员批准后才能登录主应用。"
    )
    with gr.Row():
        with gr.Column():
            reg_username = gr.Textbox(label="用户名*", placeholder="至少 3 个字符")
            reg_password = gr.Textbox(
                label="密码*", type="password", placeholder="至少 6 个字符"
            )
